...
func NewHostPathDriver(cfg Config) (*HostPath, error) {
	...
	klog.Infof("Driver: %v ", cfg)
	klog.Infof("Driver: %v ", cfg.DriverName)
	//klog.Infof("Version: %v", cfg.VendorVersion)

	s, err := state.New(path.Join(cfg.StateDir, "state.json"))
	if err != nil {
		return nil, err
	}
	quotaer, err := xfs.NewXfsQuotaer(cfg.StateDir)
	if quotaer == nil || err != nil {
		quotaer = xfs.DefaultQuotaer
	}
	hp := &HostPath{
		config:  cfg,
		state:   s,
...
func (hp *HostPath) createVolume(volID, name string, cap int64, volAccessType state.AccessType, ephemeral bool, kind string) (*state.Volume, error) {
	...
	defer func() {
		if success {
			project, u, err := hp.quotaer.AddProject(path, xfs.ConvertBytesToString(float64(cap)))
			if err != nil {
				return
			}
			fmt.Println(project, u)
		}
	}()
